...
\vspace{0.5em}
    ...
    \section{Implementation Details} \label{sec:implementation_details}
        ...
        \end{align*}
        where \(n\) can be turned into \(\emptyset\), which given that \(n_k\) can be a composite as well, allows for creation of all possible trees which the prototypes in \(\mathcal{P}\) can define. This can be seen as each tree representing a sentence in the regular grammar of the alphabet \(\mathcal{P}\).
        ...
        \end{algorithm}
        \vspace{-0.6em}
        The score is also divided by the number of round each individual played during one generation. This allows for a given individual to test its' fitness against multiple adversaries to get a better estimated fitness.
    ...
    \section{Results and Screenshots} \label{sec:results_and_screenshots}
        ...
        \end{figure}
        As can be seen in the image above. The tree is way to big to provide any significant insight unless interactively evaluated during run time. It manages however to reach a fitness score very close to the limit on all the levels it was evaluated on. When tested against a human player it becomes very hard to defeat.
        ...
        \end{figure}
        The tree manages to defeat the hard coded bot on al levels without bloating to unmanageable size. Prior exploration of generated trees indicated that as long as the tree have a \textit{sequence} of \textit{pick-up shotgun} and \textit{attack player}, it would reach an fitness of around \(1000\).
        ...
        Looking at the general fitness over generations we can observe how once the behaviour trees routinely defeats its adversary, the mean fitness plateaus but the fitness variance fills the entire range of possible fitness values. Looking at the general fitness when evaluating each tree against random peers, we see something similar but it converges with a slower rate. 
    ...
    \begin{itemize}
        ...
        \item Resulting behaviour trees
    \end{itemize}
