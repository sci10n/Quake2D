...
\vspace{0.5em}
    ...
    \section{Results and Screenshots} \label{sec:results_and_screenshots}
        ...
        As can be observed on in the bar-plot below. The fitness score increase significantly during the first couple of generations to later converge. The variance for each generation is very high which means that the individuals performs different from each other, signifying that the \textit{mutation} and \textit{cross-over} produces different offspring. This is generally a good thing since we always want as different approaches being tested for each generation to prevent getting stuck in a local maxima.

        \textbf{Graph of fitness over generations}

        \subsection{Survival Rate} \label{sec:survival_rate}

        The survival rate increases and converges in the same number of generations as the fitness score. This is expected since the fitness score gets raised when an individual manges to defeat the hard-coded behaviour tree. 

        \textbf{Graph of survival rate over generations}
