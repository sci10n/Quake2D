...
\vspace{0.5em}
    ...
    \section{Implementation Details} \label{sec:implementation_details}
        ...
        Given the very limited effective implementation time of this project. The architecture of the game is designed to allow for fast prototyping and not necessarily based on strong and scalable software design philosophies. The structural design is based on the \textit{entity component system} design which has seen high use in the contemporary games industry, being used in projects such as the \textit{Unity} game engine. The primary reference implementation is taken from the book \textit{Game programming patterns} writen by \textit{R. Nystrom}~\cite{nystrom2014game} in conjunction with \textit{M. Boström} design from \textit{Speed coding Zelda}~\footnote{\url{https://github.com/MilleBo/SpeedCodingZelda}}. The resulting implementation can be seen in the source code in the package \textit{se.sciion.quake2d.level}.
        ...
        All BTs generated for the \textit{genetic programming} optimization has to be generated during \textit{runtime}. This is done by creating an set \(\mathcal{P}\) with a \textit{prototype object} of each \textit{behaviour node} \(p\).

        Each \textit{behaviour node prototype} contains a function which causes the node to randomize it self \(\rho(p)\). For leaf nodes this means tweaking parameters such as \textit{thresholds} and \textit{entity tags}
        \begin{equation*}
            \rho(p) \rightarrow \hat{p}
        \end{equation*}

        But for composite nodes, it involves changing the children \(\{n_0,n_1,...,n_m\} \in p\) on the form:
        \begin{equation*}
